    ...
    for packet_num in range(total_packets):
        ...
        if packet.haslayer('IP'):
            ...
            if src_ip not in os_mapping:
                os_mapping[src_ip] = os_fingerprint(packet)
...
def plot_map(packets):
    if not packets:
        print("No packets to analyze")
        return

    os_mapping, dns_servers, ip_packets = analyze_packets(packets)
...
def netmap():
    packets = open_pcap('/home/kali/Documents/Networksniff/trace1.pcapng')
    if packets:
        plot_map(packets)
